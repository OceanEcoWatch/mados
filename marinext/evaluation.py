...
from utils.assets import bool_flag, labels
from utils.dataset import MADOS, bands_mean, bands_std
from utils.metrics import Evaluation, confusion_matrix
from utils.test_time_aug import TTA

root_path = up(up(os.path.abspath(__file__)))

logging.basicConfig(
    filename=os.path.join(root_path, "logs", "evaluating_marinext.log"),
    filemode="a",
    level=logging.INFO,
...
def main(options):
    ...
    with torch.no_grad():
        for image, target in tqdm(test_loader, desc="testing"):
            if (
                options["test_time_augmentations"] and options["batch"] == 1
            ):  # Only with batch = 1
                image = TTA(image)
        ...
        conf_mat = confusion_matrix(
            y_true, y_predicted, labels, options["results_percentage"]
        )
        ...
        if options["predict_masks"]:
            ...
            ROIs_split = np.genfromtxt(
                os.path.join(options["path"], splits_path, options["split"] + "_X.txt"),
                dtype="str",
            )
            ...
            for tile in tqdm(tiles, desc="testing"):
                ...
                splits = [
                    f.split("_cl_")[-1]
                    for f in glob(os.path.join(tile, "10", "*_cl_*"))
                ]
                ...
                for crop in splits:
                    ...
                    if crop_name in ROIs_split:
                        ...
                        for c, band in enumerate(all_bands, 1):
                            upscale_factor = (
                                int(os.path.basename(os.path.dirname(band))) // 10
                            )
                        ...
                        output_image = os.path.join(
                            ...
                            os.path.basename(crop_name).split(".tif")[0]
                            + "_marinext.tif",
                        )
                        ...
                        with rasterio.open(
                            ...
                            dtype=image.dtype,
                            crs="+proj=latlong",
                        ) as dst:  # non-georeferenced (just to be recognised from gis)
                            ...
                            for model in models_list:
                                ...
                                logits = model(image)
                                logits = F.upsample(
                                    input=logits, size=(240, 240), mode="bilinear"
                                )
                                ...
                                if options["test_time_augmentations"]:
                                    predictions = TTA(
                                        predictions, reverse_aggregation=True
                                    )
                                all_predictions.append(predictions)

                            all_predictions = torch.cat(all_predictions)
                            all_predictions = torch.mode(
                                all_predictions, dim=0, keepdim=True
                            )[0]

                            predictions = predictions.squeeze().cpu().numpy()
                            # Write the mask with georeference
                            dst.write_band(
                                1, predictions.astype(dtype).copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--split", default="test", type=str, help="Which dataset split (test or val)"
    )
    parser.add_argument(
        "--test_time_augmentations",
        default=True,
        type=bool_flag,
        help="Generate maps and score based on multiple augmented testing samples? (Use batch = 1 !!!) ",
    )
    ...
    parser.add_argument(
        "--output_channels", default=15, type=int, help="Number of output classes"
    )
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            up(os.path.abspath(__file__)), "trained_models", "45", "model_ema.pth"
        ),
    ...
    parser.add_argument(
        ...
        default=True,
        type=bool_flag,
        help="Generate confusion matrix results in percentage?",
    )
    ...
    parser.add_argument(
        "--gen_masks_path",
        default=os.path.join(root_path, "data", "predicted_marinext"),
